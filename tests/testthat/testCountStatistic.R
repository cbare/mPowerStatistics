...
tap<-function(time, x, y, lrn) {
	if (lrn=="l") {
		id<-"TappedButtonLeft"
	} else if (lrn=="r") {
		id<-"TappedButtonRight"
	} else if (lrn=="n") {
		id<-"TappedButtonNone"
	} else {
		stop("Unknown value for lrn: ", lrn)
	}
	tap<-list(TapTimeStamp=time, TappedButtonId=id, TapCoordinate=sprintf("{%s, %s}",x,y))
}

expect_equal(
		ShapeTappingData(list(tap(0.1,1,2,'l'),tap(0.2,3,4,'r'),tap(0.3,5,6,'n'))),
		data.frame(
				time=c(.1,.2,.3), 
				X=c(1,3,5), 
				Y=c(2,4,6), 
				buttonid=c("TappedButtonLeft","TappedButtonRight","TappedButtonNone"), 
				stringsAsFactors=F)
)

# Happy case:  3 taps
expect_equal(tappingCountStatistic(list(tap(0.1,1,2,'l'),tap(0.2,3,4,'r'),tap(0.3,5,6,'n'))), 3)

# Repeated taps, but all left or right
expect_equal(tappingCountStatistic(list(tap(0.1,1,2,'l'),tap(0.2,1,2,'r'),tap(0.3,1,2,'l'))), 3)

# Repeated taps, none
expect_equal(tappingCountStatistic(list(tap(0.1,1,2,'n'),tap(0.2,1,2,'n'),tap(0.3,1,2,'n'))), 1)
...
# These are purely regression tests.  They don't test correctness, just whether
# the output is consistent.
testTaps(155, "tapping_results.json.TappingSamples-1a642746-7f22-421b-a6de-d90eabf99ba96278340256554412503.json")
testTaps(129, "tapping_results.json.TappingSamples-2e87c348-d2e1-450f-9c34-2eab52fda5af111790938118683279.json")
...
testTaps(217, "tapping_results.json.TappingSamples-b2bafa3c-3178-46fd-81d3-c1a0e0a8ab965371216142531495452.json")
testTaps(132, "tapping_results.json.TappingSamples-bac511da-1c4a-40e7-a4a9-2770bd74c1178230558000319275806.json")
...
testTaps(154, "tapping_results.json.TappingSamples-d1a0ff32-6756-4c65-bd7b-d5b60faec3926194198098794520520.json")
testTaps(32, "tapping_results.json.TappingSamples-dc348d0b-2457-4394-8bea-86b5975a92a68229272086582075256.json")
testTaps(208, "tapping_results.json.TappingSamples-e255a421-36f0-4fd8-b548-dd67dedb445a8941215721774187059.json")
testTaps(159, "tapping_results.json.TappingSamples-e5cb5f69-a75a-49cc-83b4-dd0871a6b4fb6002873297461980535.json")
